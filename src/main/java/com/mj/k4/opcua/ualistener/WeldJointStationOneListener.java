...
@Slf4j
@Component
public class WeldJointStationOneListener implements MonitoredDataItemListener {
    @Inject
    ...
    @Async("taskExecutor")
    private void weldJointStationOneSixWebsocket(MonitoredDataItem monitoredDataItem, DataValue dataValue, DataValue dataValue1) {
        ...
        if (dataValue != null && dataValue.getValue().getValue() != dataValue1.getValue().getValue() && (boolean) dataValue.getValue().getValue() && !(boolean) dataValue1.getValue().getValue()) {
            log.info("熔接工位1急停恢复被触发");
        }
    }

    @Override
    public void onDataChange(MonitoredDataItem monitoredDataItem, DataValue dataValue, DataValue dataValue1) {
        log.info("熔接工位1被触发 OperationRoomListener monitoredDataItem= " + monitoredDataItem + ";dataValue=" + dataValue + ";dataValue1=" + dataValue1);
        taskExecutor.execute(() -> weldJointStationOneSixWebsocket(monitoredDataItem, dataValue, dataValue1));
    }
}
