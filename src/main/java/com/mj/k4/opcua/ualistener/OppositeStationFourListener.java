...
@Slf4j
@Component
public class OppositeStationFourListener  implements MonitoredDataItemListener {
    ...
    @Async("taskExecutor")
    private void oppositeStationFourWebsocket(MonitoredDataItem monitoredDataItem, DataValue dataValue, DataValue dataValue1) {
        ...
        if (dataValue != null && dataValue.getValue().getValue() != dataValue1.getValue().getValue() && (boolean) dataValue.getValue().getValue() && !(boolean) dataValue1.getValue().getValue()) {
            log.info("反面工位4急停恢复被触发");
        }
    }
    @Override
    public void onDataChange(MonitoredDataItem monitoredDataItem, DataValue dataValue, DataValue dataValue1) {
        log.info(" 反面工位4被触发 AffuseMachineListener monitoredDataItem= " + monitoredDataItem + ";dataValue=" + dataValue + ";dataValue1=" + dataValue1);
        taskExecutor.execute(() -> oppositeStationFourWebsocket(monitoredDataItem, dataValue, dataValue1));
    }
}
