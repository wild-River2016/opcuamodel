...
@Slf4j
@Component
public class PositiveStationSixListener implements MonitoredDataItemListener {
    ...
    @Async("taskExecutor")
    private void positiveStationSixWebsocket(MonitoredDataItem monitoredDataItem, DataValue dataValue, DataValue dataValue1) {
        ...
        if (dataValue != null && dataValue.getValue().getValue() != dataValue1.getValue().getValue() && (boolean) dataValue.getValue().getValue() && !(boolean) dataValue1.getValue().getValue()) {
            log.info("正面工位6急停恢复被触发");
        }
    }

    @Override
    public void onDataChange(MonitoredDataItem monitoredDataItem, DataValue dataValue, DataValue dataValue1) {
        log.info("正面工位6被触发 OperationRoomListener monitoredDataItem= " + monitoredDataItem + ";dataValue=" + dataValue + ";dataValue1=" + dataValue1);
        taskExecutor.execute(() -> positiveStationSixWebsocket(monitoredDataItem, dataValue, dataValue1));
    }
}
