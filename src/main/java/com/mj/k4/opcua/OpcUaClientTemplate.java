...
import com.mj.k4.opcua.util.PlcConnectionStatus;
...
public class OpcUaClientTemplate {
    ...
    public void connectAlwaysInBackend() {
        ...
        for(int i=0;i<uaClientList.size();i++){
            try {
                UaClient uaClient=uaClientList.get(i);
                connect(uaClient);
                fireConnectionListeners(i,uaClient);
            } catch (OpcUaClientException e) {
    ...
    public synchronized boolean connect(UaClient uaClient) throws OpcUaClientException {
        if (!uaClient.isConnected()) {
            try {
                ...
                System.out.println("服务端是否为空  :"+uaClient);
                uaClient.connect();
                if(uaClient.getUri().contains("213.211")){PlcConnectionStatus.PLC01STATUS=true;}
                if(uaClient.getUri().contains("213.213")){PlcConnectionStatus.PLC02STATUS=true;}
                if(uaClient.getUri().contains("213.159")){PlcConnectionStatus.PLC03STATUS=true;}
                if(uaClient.getUri().contains("213.189")){PlcConnectionStatus.PLC04STATUS=true;}
                return true;
            } catch (Exception e) {
                LOGGER.error("Error connecting ua server:{}", uaClient.getUri());
                //通过异常设置PLC是否连接的状态
                if(uaClient.getUri().contains("213.211")){PlcConnectionStatus.PLC01STATUS=false;}
                if(uaClient.getUri().contains("213.213")){PlcConnectionStatus.PLC02STATUS=false;}
                if(uaClient.getUri().contains("213.159")){PlcConnectionStatus.PLC03STATUS=false;}
                if(uaClient.getUri().contains("213.189")){PlcConnectionStatus.PLC04STATUS=false;}
